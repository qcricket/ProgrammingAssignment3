rankall <- function(outcome, num = "best") {
  ...
  ## IF outcome = "heart attack":
  ## Cut observations with Not Available
  #df.HA <- df[df$HA != "Not Available", ]
  
  ## Split observations by state
  f <- factor(df.HA$State)
  df.HA.Split <- split(df.HA, f)
  
  ## Rank in ascending order by mortality rate
  df.HA.Ordered <- lapply(x function(x) x[order(x$HA, x$Name), ])
  
  ## Order each state
  df.HA.Ordered <- lapply(df.HA.Split, function(x) x[order(x$HA, x$Name), ])
  ...
  ## Cut observations with Not Available
  t <- lapply(t, function(x) x[x$outcome!="Not Available", ])
  
  ranked <- lapply(t, function(x) x[order(x$"heart attack", x$"Hospital.Name", na.last = NA), ])
